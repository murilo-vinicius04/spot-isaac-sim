...
| `cameras.py` | Spot's 6 cameras (hand + front-left/right + left/right/rear), same poses and lenses as `applications/spot_cameras.py`, authored as USD cameras under the links by `make_spot_stage.py`. `walk.py --camera-snap-at T` saves one image from each |
...
Drive it in the warehouse (the Isaac window opens on your display; click it, then use the keys):
...
right 0.37 and left 0.34 of 0.40 m/s, and it stops when the keys are released. The policy only ever saw one command
per episode, so changing commands while walking is outside its training. It handled it in these tests.
